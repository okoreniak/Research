...
UshareWrapper::UshareWrapper(const QString &UshareUrl, QObject *parent) :
    mBaseUshareUrl(UshareUrl),
	QObject(parent),
    mAccessManager(new QNetworkAccessManager(this))
{
    connect(mAccessManager, &QNetworkAccessManager::finished, this, &UshareWrapper::handleReply);
 ...
 * to the image.
 */
void UshareWrapper::startUpload(const QImage& image, const QByteArray& accessToken) const
{
    ...
    //QUrl url("https://spixel.phonexa.xyz/test-ushare-desktop/upload");
    QUrl url("https://ushare.phonexa.com/api/desktop-screenshot");
    QUrlQuery urlQuery;
    ...
    qDebug() << "request sent " << request.url() ;
}
 ...
 * the tokenError.
 */
void UshareWrapper::getAccessToken(const QByteArray& pin, const QByteArray& clientId, const QByteArray& clientSecret) const
{
    QNetworkRequest request;

    // Build the URL that we will request the token from. The XML indicates we
    // want the response in XML format.
    request.setUrl(QUrl(mBaseUshareUrl + QStringLiteral("/oauth2/token.xml")));
    request.setHeader(QNetworkRequest::ContentTypeHeader, QStringLiteral("application/x-www-form-urlencoded"));
...
/*
 * The Ushare token expires after some time, when this happens and you try to
 ...
 * refresh the token.
 */
void UshareWrapper::refreshToken(const QByteArray& refreshToken, const QByteArray& clientId, const QByteArray& clientSecret) const
{
    QNetworkRequest request;
 ...
 * correct url to it.
 */
QUrl UshareWrapper::pinRequestUrl(const QString& clientId) const
{
    QUrl url(mBaseUshareUrl + QStringLiteral("/oauth2/authorize"));
    QUrlQuery urlQuery;
 ...
 */
void UshareWrapper::handleDataResponse(const QDomElement& element) const
{
    qDebug() << element.text();
    if (element.attribute(QStringLiteral("status")) == QStringLiteral("200") && !element.elementsByTagName(QStringLiteral("link")).isEmpty()) {
        ...
        emit uploadFinished(UshareResponse(link, deleteHash));
 ...
 */
void UshareWrapper::handleTokenResponse(const QDomElement& element) const
{
...
/*
 * This function will be called when we've got the reply from Ushare
 */
void UshareWrapper::handleReply(QNetworkReply* reply)
{
...
//    qDebug("----------------------------------------------------------------");
    ...
    if (rootElement.tagName() == QStringLiteral("data")) {
        ...
        emit error(QStringLiteral("Received unexpected reply from Ushare server."));
    }
